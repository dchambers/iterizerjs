...
// TODO: see if we could instead do `not.equals()` like Jasmine does
Iterizer.prototype.not = function(filterFunc) {
...
Iterizer.prototype.installGlobals = function() {
	...
	for(var methodName of methodNames.filter(this.not(this.beginsWith('install')))) {
...
Iterizer.prototype.installMethods = function() {
	Object.defineProperty(String.prototype, 'chars', {enumerable: false, configurable: true, value: function*() {
		if(this.length > 0) {
			for(var i of it.range(0, this.length - 1)) {
				yield this.charAt(i);
			}
		}
	}});
